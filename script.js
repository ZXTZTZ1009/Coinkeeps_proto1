...
document.getElementById("calcButton").addEventListener("click", () => {
    ...
    // Set both to midnight to prevent daylight savings/timezone math bugs
    fromDate.setHours(0,0,0,0);
    toDate.setHours(0,0,0,0);

    // CALENDAR LOOP: Count exact days and exact weekdays
    let totalDays = 0;
    let exactWeekdays = 0;
    ...
    while (currentDate <= toDate) {
        ...
        let dayOfWeek = currentDate.getDay(); // 0 is Sunday, 1 is Mon, 6 is Sat
        if (dayOfWeek !== 0 && dayOfWeek !== 6) {
            exactWeekdays++; // It is a weekday!
        }
    ...
    expenses.forEach(e => {
        let multiplier = 1;
        switch(e.frequency) {
            case "Everyday": multiplier = totalDays; break;           // PERFECT MATCH
            case "Weekdays": multiplier = exactWeekdays; break;       // PERFECT MATCH
            case "Once": multiplier = 1; break;                       // PERFECT MATCH
            
            // For arbitrary days, proportional averages are still mathematically best!
...
async function fetchExpenses() {
    // Safety check: ensure we have a valid logged-in user with an ID
    ...
    try {
        ...
        if (json.status === "success") {
            // Update the global expenses array with the downloaded data
            expenses = json.data; 
...
function renderExpenseList() {
    ...
    list.innerHTML = ""; // Clear existing visual list
    
    const currency = document.getElementById("currencyTypeInput").value;

    // Loop through the global array and build the list items
    expenses.forEach(exp => {
        ...
        removeBtn.addEventListener("click", () => {
            // Remove from local array and screen
            expenses = expenses.filter(e => e.id !== exp.id);
            li.remove();
            
            // NEW: Tell the backend to delete it from the Google Sheet
            if (currentUser) {
                sendRequest({
...
document.getElementById("clearAllBtn").addEventListener("click", () => {
    if(confirm("Are you sure you want to clear all expenses?")) {
        // Ping backend to delete each one
        if (currentUser) {
            expenses.forEach(exp => {
